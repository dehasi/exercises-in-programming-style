...
# [(w1,1), ...]
# returns a list of pairs [(w1, frequency), ...],
# where 'frequency' is the sum of all reported occurrences
def count_words(pairs_list_1, pairs_list_2)
  mapping = Hash.new(0)
  [pairs_list_1, pairs_list_2].each do |pl|
    pl.each { |p| mapping[p[0]] += p[1] }
  end
  mapping.to_a
end
...
word_freqs = sort(splits.reduce { |pairs1, pairs2| count_words(pairs1, pairs2) })
